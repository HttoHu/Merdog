...
namespace Mer
{
    ...
    namespace Parser
    {
        ...
        */
        ...
        {
            using namespace BranchAndLoop;
            Env::skip_block_begin = true;
            PosPtr end_pos = std::make_shared<size_t>(0), start_pos = std::make_shared<size_t>(0), target_pos = std::make_shared<size_t>(0);
            ...
            Env::cur_ins_table->push_back(UptrPNode(var_decl));
            Env::cur_ins_table->push_back(std::make_unique<Goto>(Env::cur_pc, target_pos));
            *start_pos = Env::get_cur_block_pos();
            Env::cur_ins_table->push_back(UptrPNode(step));

            *target_pos = Env::get_cur_block_pos();
            Env::cur_ins_table->push_back(std::make_unique<IfFalseJmp>(Env::cur_pc, condition, end_pos));
            parse_auto_block_part();
            Env::cur_ins_table->push_back(std::make_unique<Goto>(Env::cur_pc, start_pos));

            *end_pos = Env::get_cur_block_pos();
            Env::end_block();
            Env::end_loop();
        }
        /*
        ...
        void build_if()
        {
            ...
            Env::new_block();
            auto branch = new IfFalseJmp(Env::cur_pc, condition,end_pos);
