...
std::map<std::string, void (*)()> Mer::repository{
    {"vector", Mer::Container::using_vector}, {"deque", Mer::Container::using_deque}, {"set", Mer::using_set}, {"map", Mer::using_map}};
...
namespace Mer
{
    std::unique_ptr<SharedObjEnv> so_env = nullptr;
}
namespace Mer
{
    ...
    std::string run_interpreter(const std::string &file_content)
    {
        ...
        so_env = std::make_unique<SharedObjEnv>();
